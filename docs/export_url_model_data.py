...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export URL datasets and URL model outputs to Excel for manual review."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def export_url_review(output_path: Path | None = None) -> Path:
    repo_root = resolve_repo_root()
    processed_dir = repo_root / "data" / "processed"
    models_dir = repo_root / "models" / "url"

    dataset_specs = [
        ("url_model_dataset", processed_dir / "url_model_dataset.parquet"),
        ("url_train", processed_dir / "url_train.parquet"),
        ("url_val", processed_dir / "url_val.parquet"),
        ("url_test", processed_dir / "url_test.parquet"),
    ]
    ...
    model_frames = build_run_summary_frames(models_dir, "url")
    if not loaded_frames and not model_frames:
        raise FileNotFoundError("No URL datasets or URL model outputs were found to export.")

    final_output_path = resolve_output_path(output_path, "url_dataset_review")
    summary_rows: list[dict[str, object]] = []
    ...
    with pd.ExcelWriter(final_output_path, engine="openpyxl") as writer:
        if loaded_frames:
            ...
            write_sheet(
                writer,
                "url_dataset_overview",
                overview_df,
                summary_rows,
                str(processed_dir),
                "Generated summary for URL datasets.",
            )

            main_frame = next((df for name, _, df in loaded_frames if name == "url_model_dataset"), None)
            if main_frame is not None:
                ...
                if not by_source_df.empty:
                    write_sheet(
                        ...
                        by_source_df,
                        summary_rows,
                        str(processed_dir / "url_model_dataset.parquet"),
...
def main() -> None:
    args = parse_args()
    output_path = export_url_review(args.output)
    print(f"URL review Excel file saved to: {output_path}")
